...
struct TaggedVirtualMethodTablePointer(u64);
...
impl TaggedVirtualMethodTablePointer
{
	...
	#[inline(always)]
	pub(crate) fn tag(tag_bits: u32, virtual_method_table_pointer: VirtualMethodTablePointer) -> Self
	{
	...
	/// Tag bits must not exceed `2^19 - 1` to be platform-portable.
	#[inline(always)]
	pub(crate) fn special(tag_bits: u32) -> Self
	{
	...
	fn new(tag_bits: u32, virtual_method_table_pointer: *mut ()) -> Self
	{
		...
		let virtual_method_table_pointer_downshifted = (virtual_method_table_pointer.as_ptr() as usize as u64) >> AlignedPointerShift;
	...
	#[inline(always)]
	pub(crate) fn virtual_method_table_pointer(self) -> Option<NonNull<()>>
	{
		let raw_pointer = ((self.0 & !Self::TagBitsMask) << Self::AlignedPointerShift) as usize as *mut ();
		if unlikely!(raw_pointer.is_null())
		{
			None
		}
		else
