...
impl ResourceRecord
{
	...
	fn handle_ipseckey<'a>(&'a self, end_of_name_pointer: usize, end_of_message_pointer: usize, resource_record_name: ParsedNameIterator<'a>, resource_record_visitor: &mut impl ResourceRecordVisitor) -> Result<usize, DnsProtocolError>
	{
		...
		if unlikely!(resource_data.len() < 3)
		{
			return Err(ResourceDataForTypeIPSECKEYHasTooShortALength(resource_data.len()))
		}

		let precedence = unsafe { *(resource_data.get_unchecked(0)) };
		let gateway_type = unsafe { *(resource_data.get_unchecked(1)) };
		let public_key_algorithm_type = unsafe { *(resource_data.get_unchecked(2)) };
		...
		let (public_key_starts_at_offset, gateway) = match gateway_type
		{
			...
			1 =>
			{
				if unlikely!(resource_data.len() < GatewayFieldStartsAtOffset + 4)
				{
					return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForAnInternetProtocolVersion4Gateway(resource_data.len()))
				}
				let gateway: IpAddrV4 = unsafe { transmute(*(resource_data.get_unchecked(GatewayFieldStartsAtOffset) as *const [u8; size_of::<IpAddrV4>()])) };
			...
			2 =>
			{
				if unlikely!(resource_data.len() < GatewayFieldStartsAtOffset + 16)
				{
					return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForAnInternetProtocolVersion6Gateway(resource_data.len()))
				}
				let gateway: IpAddrV6 = unsafe { transmute(*(resource_data.get_unchecked(GatewayFieldStartsAtOffset) as *const [u8; size_of::<IpAddrV6>()])) };
			...
			3 =>
			{
				if unlikely!(resource_data.len() < GatewayFieldStartsAtOffset + 1)
				{
					return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForDomainNameGateway(resource_data.len()))
				}

				let resource_data_starts_at_pointer = resource_data.as_ptr() as usize;
				let start_of_name_pointer = resource_data_starts_at_pointer + GatewayFieldStartsAtOffset;
				let (domain_name, end_of_domain_name_pointer) = ParsedNameIterator::parse_without_compression(start_of_name_pointer, start_of_name_pointer + resource_data.len() - GatewayFieldStartsAtOffset)?;

				(end_of_domain_name_pointer - resource_data_starts_at_pointer, Some(DomainName(domain_name)))
			}

			_ => return Err(ResourceDataForTypeIPSECKEYHasAnUnknownGatewayType(gateway_type))
		};
		...
		{
			0 => if unlikely!(public_key_data.len() != 0)
			{
				return Err(ResourceDataForTypeIPSECKEYHasWrongLengthForNoPublicKey(resource_data.len()))
			}
			else
			{
				None
			...
			1 =>
			{
				...
				{
					return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForADSAPublicKey(resource_data.len()))
				}

				let T = unsafe { * public_key_data.get_unchecked(0) };
				...
				{
					return Err(ResourceDataForTypeIPSECKEYHasWrongLengthForADSAPublicKeyOnceTIsKnown(resource_data.len()))
				}
			...
			2 =>
			{
				...
				{
					return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForRSAPublicKey(resource_data.len()))
				}

				let first_byte_of_exponent_length = unsafe { * public_key_data.get_unchecked(0) };
				let (exponent_and_modulus, exponent_length) = if first_byte_of_exponent_length == 0
				{
					if unlikely!(public_key_data.len() < 3)
					{
						return Err(ResourceDataForTypeIPSECKEYHasTooShortALengthForRSAPublicKeyForAThreeByteExponentLength(resource_data.len()))
					}

					(&public_key_data[3 .. ], u16::from_be_bytes(unsafe { * public_key_data.get_unchecked(1) as *const [u8; 2] }) as usize)
				}
				else
				{
			...
			3 =>
			{
				if unlikely!(public_key_data.len() != 256 / 8 || public_key_data.len() != 384 / 8)
				{
			...
			_ => Some(Unknown { public_key_algorithm_type, public_key_data })
		};
