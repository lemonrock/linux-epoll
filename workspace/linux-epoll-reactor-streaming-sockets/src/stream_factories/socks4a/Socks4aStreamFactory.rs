...
impl<SF: StreamFactory<SD>, SD: SocketData> StreamFactory<SD> for Socks4aStreamFactory<SF, SD>
{
	...
	/// Returns a bound socket address; this is usually garbage for SOCKS4 (but not SOCKS4a) connections.
	type ProxyOrTunnelInformation = (SocketAddrV4, SF::ProxyOrTunnelInformation);
	...
	fn new_stream_and_handshake<'yielder>(&self, streaming_socket_file_descriptor: StreamingSocketFileDescriptor<SD>, yielder: Yielder<'yielder, ReactEdgeTriggeredStatus, (), Result<(), CompleteError>>, additional_arguments: Self::AdditionalArguments) -> Result<(Self::S, Self::ProxyOrTunnelInformation), CompleteError>
	{
		...
		let (unencrypted_stream, bound_socket) = Socks4aConnectReply::read_reply(unencrypted_stream)?;

		let (streaming_socket_file_descriptor, yielder) = unencrypted_stream.unwrap();

		let (stream, proxy_or_tunnel_information) = self.0.new_stream_and_handshake(streaming_socket_file_descriptor, yielder, additional_arguments.1);

		Ok((stream, (bound_socket, proxy_or_tunnel_information)))
	}
}
